		...
		if (noc_info[noc_idx].intr_vector == intr_vector) {
			...
			if (noc_info[noc_idx].summary_intr_status_addr != NULL) {
				if (noc_info_oem[noc_idx].summary_intr_enable_bit_set != 0u) {
					uint32_t status = mmio_read_32(
						(uintptr_t)noc_info[noc_idx].summary_intr_status_addr);
	...
	for (i = 0; i < nocerr_propdata->len; i++) {
		...
		if (!is_qti_noc_error_isr_registered(i)) {
			...
			if (err != 0) {
				...
				for (uint32_t k = 0; k < i; k++) {
					...
					if (noc_info_oem[k].intr_enable) {
						if (noc_info[k].summary_intr_enable_addr != NULL) {
							mmio_write_32((uintptr_t)noc_info[k].summary_intr_enable_addr,
								  mmio_read_32((uintptr_t)noc_info[k].summary_intr_enable_addr) &
								  ~noc_info_oem[k].summary_intr_enable_bit_set);
						}
						if (noc_info[k].base_addr != NULL) {
							mmio_write_32(NOC_REG_ADDR(noc_info[k].base_addr,
									       noc_info[k].hw->main_ctl_low),
								  0x0);
						}
					}
				}
		...
		if (noc_info_oem[i].intr_enable) {
			if (noc_info[i].summary_intr_enable_addr != NULL) {
				mmio_write_32((uintptr_t)noc_info[i].summary_intr_enable_addr,
					  mmio_read_32((uintptr_t)noc_info[i].summary_intr_enable_addr) |
